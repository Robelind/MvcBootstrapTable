...
{
    internal interface ITableRenderer<T> where T : new()
    {
        IHtmlContent Render(ITableConfig<T> config, int entityCount);
    }

    internal class TableRenderer<T> : ITableRenderer<T> where T : new()
    {
        private readonly TableState _tableState;
        private readonly ITableNodeParser _nodeParser;
        private ITableConfig<T> _config;
        ...
        private void Body(TableNode table)
        {
            ...
            // Rows.
            foreach(RowConfig<T> rowConfig in _config.Rows)
            {
        ...
        private void Header(TableNode table)
        {
            if(_config.Columns.Values.Any(c => !string.IsNullOrEmpty(c.Header)))
            {
                ...
                // Columns.
                this.IterateProperties(entity, (propInfo, config) =>
                {
        ...
        private void Footer(TableNode table)
        {
            if(_config.Paging.PageSize > 0 || !string.IsNullOrEmpty(_config.Footer.Text))
            {
                ...
                if(footerContent.Any())
                {
                    ...
                    int colCount = new T().GetType().GetProperties()
       ...
       private void SetupAjaxAttrs(TagBuilder builder, string queryAttr = null)
        ...
        private void AddAttribute(TagBuilder builder, string attribute, string value)
        {
            if(!string.IsNullOrEmpty(value))
            {
                builder.Attributes.Add(attribute, value);
            }
        }

        private void AddCssClasses(TagBuilder builder, IEnumerable<string> cssClasses)
        {
